...
eval_basis <- function(evalarg, basis, Lfdobj = 0,
                       sparse_matrix = TRUE) {
  if (is(basis, "basisfd")) {
    ...
    if (length(Lfdobj) == 1) Lfdobj <- rep(Lfdobj, D)
...
get_basis_inprod_matrix <- function(basis, sparse_matrix = TRUE) {
  if (is(basis, "basisfd")) {
    ...
    if (sparse_matrix) inprodmat <- as(inprodmat, "sparseMatrix")
    return(inprodmat)
  } else if (is(basis, "TensorBasis")) {
    D <- length(basis)
...
get_basis_penalty_matrix <- function(
    basis, penLfd = 2, sparse_matrix = TRUE, sum_matrices = TRUE) {
  if (is(basis, "basisfd")) {
...
smooth_basis <- function(argvals, Y, basis, lambda = 0, penLfd = 2) {
  ...
  coef <- as.matrix(Matrix::solve(
    Matrix::crossprod(B) + lambda * S, Matrix::crossprod(B, Y)))
  if (is(basis, "basisfd")) {
...
smooth_basis.gcv <- function(argvals, Y, basis, lambda = 0, penLfd = 2) {
  # TODO: faster computation by utilizing
  # eigendecomposition properties of kronecker products and kronecher sums
  ...
  return(list(lambda = lambda, GCV = GCV,
              coef = coef, lambda.min = lambda[idx],
              fd = fdObj))
}
