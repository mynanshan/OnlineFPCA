...
FB.HB.all <- function(B, phi.aux, sig, Lambda, n, index) {
  ...
  temp <- apply(matrix(1:n), MARGIN = 1, FB.HB, B = B,
                phi.aux = phi.aux, sig = sig, Lambda = Lambda, index = index)
...
F.Lambda <- function(B, phi.aux, sig, Lambda, data.list, i) {
  ...
  for (k in 1:r) {
    ...
    temp1.0k <- 2 * sig^2 * Lambda[k] *
      (t.y %*% P.inv %*% t.phi %*% bk) * (t.y %*% P.inv2 %*% t.phi %*% bk)
    lk <- t.bk %*% phi %*% P.inv
    temp2.0k <- (-2 * sig^2 * Lambda[k]) * lk %*% t(lk)
    Hess[1, k + 1] <- temp1.0k + temp2.0k
    Hess[k + 1, 1] <- Hess[1, k + 1]

    temp1.kk <- temp1.k + 2 * Lambda[k]^2 *
      (t.y %*% P.inv %*% t.phi %*% bk)^2 * (t.bk %*% phi %*% P.inv %*% t.phi %*% bk)
    ...
    for (l in 1:k) {
      if (l != k) {
        bl <- B[, l]
        temp1.kl <- 2 * Lambda[k] * Lambda[l] *
          (t.y %*% P.inv %*% t.phi %*% bk) * (t.y %*% P.inv %*% t.phi %*% bl) * (t(bl) %*% phi %*% P.inv %*% t.phi %*% bk)
        temp2.kl <- (-Lambda[k] * Lambda[l]) *
          (t.bk %*% phi %*% P.inv %*% t.phi %*% bl)^2
...
F.Lambda.all <- function(B, phi.aux, sig, Lambda, data.list, n) {
  r <- ncol(B)
  temp <- apply(
    matrix(1:n), MARGIN = 1, F.Lambda, B = B, phi.aux = phi.aux,
    sig = sig, Lambda = Lambda, data.list = data.list)
...
Newton.New <- function(B.c, phi.aux, sig.c, Lambda.c, data.list, n,
                       sl.v, max.step = 50, tol = 1e-3, cond.tol = 1e+10) {
  M <- nrow(B.c)
  r <- ncol(B.c)
  ...
  while (i < max.step && tol.c >= tol) {
    ...
    ## update lambda and sig
    temp.Lam <- try(
      Update.Lambda(B.c, phi.aux, sig.c, Lambda.c, data.list, n, epsi = 1e-6))
  ...
  ## return
  result <- list(like, B.result, lam.result, sig.result,
                 gradB.result, gradL.result, i, cond, error.c)
  return(result)
}
...
loglike.all <- function(B, phi.aux, sig, Lambda, data.list, n) {
  temp <- apply(matrix(1:n), MARGIN = 1, loglike, B = B,
                phi.aux = phi.aux, sig = sig, Lambda = Lambda, data.list = data.list)
  result <- mean(temp)
  return(result)
}
...
loglike.cov.all <- function(covmatrix, sig, data.list, n) {
  temp <- apply(matrix(1:n), MARGIN = 1, loglike.cov,
                covmatrix = covmatrix, sig = sig, data.list = data.list)
  result <- mean(temp)
  return(result)
}
...
CV.B <- function(B, phi.aux, sig, Lambda, n) {
  ...
  ## (i) H^{-1}:
  temp <- apply(matrix(1:n), MARGIN = 1, FB.HB, B = B,
                phi.aux = phi.aux, sig = sig, Lambda = Lambda, index = index.t)
...
CV.Lambda <- function(B, phi.aux, sig, Lambda, data.list, n) {
  ...
  r <- ncol(B)
  temp <- apply(matrix(1:n), MARGIN = 1, F.Lambda, B = B,
                phi.aux = phi.aux, sig = sig, Lambda = Lambda, data.list = data.list)
...
#################################### VII: initial values
Initial <- function(r, ini.method, data.list, n, nmax, grid.l,
                    grids, M.EM = 25, iter.num = 50, basis.EM = "ns", sig.EM) {
  band <- NULL
  if (ini.method == "loc") {
