...
fpca.mle <- function(data.m, M.set, r.set, ini.method = "EM",
                     basis.method = "bs", sl.v = rep(0.5, 10), max.step = 50,
  ...
  for (k in 1:length(r.set)) {
    ...
    print(paste("r=", r.c))
    result.c <- fpca.fit(
      M.set, r.c, data.list.new, n, nmax, grid.l, grids,
      iter.EM.num, sig.EM, ini.method, basis.method,
  ...
  temp <- list("selected_model" = temp.model, "eigenfunctions" = eigenf.sele,
               "eigenvalues" = eigenv.sele, "error_var" = sig.sele^2,
               "fitted_mean" = fitmu, "grid" = grids.new,
               "cv_scores" = cv.result, "converge" = con.result)
...
fpca.fit <- function(M.set, r, data.list, n, nmax,
                     grid.l = seq(0, 1, 0.01), grids = seq(0, 1, 0.002),
                     iter.num = 50, sig.EM = 1, ini.method, basis.method = "poly",
  ...
  if (ini.method == "EM.self") {
    IniVal <- try(
      Initial(r, ini.method = "EM", data.list, n, nmax,
              grid.l, grids, basis.EM = basis.EM, M.EM = M.self, sig.EM = sig.EM)
    )
    if (inherits(IniVal, "try-error")) {
  ...
  for (i in 1:M.l) {
    ...
    if (basis.method == "poly") { ## cubic B-spline with equal spaced knots
      ...
      phi.aux <- apply(
        matrix(1:n), MARGIN = 1, Phi.aux, M = M, basis.method = basis.method,
        data.list = data.list, R.inv = R.inv)
    }
    ...
    if (basis.method == "ns") { ## nature spline
      ...
      phi.aux <- apply(
        matrix(1:n), MARGIN = 1, Phi.aux, M = M, basis.method = basis.method,
        data.list = data.list, R.inv = bs.u, grid = grids)
    }

    #### (iv) Newton iterations
    newton.result <- Newton.New(B.ini, phi.aux, sig.ini, Lambda.ini,
                                data.list, n, sl.v, max.step, tol, cond.tol)
  ...
  #### (vii) return result
  result <- list(
    "eigenval" = eigen.result, "sig" = sig.result, "-2*loglike" = like.result,
