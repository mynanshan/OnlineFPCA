...
ofpca <- function(data_generator, obsGrid, meanfun = FALSE,
                  tau = NULL, tau.control = list(), 
                  nbatch = 1, sgd.control = list(),
                  stepsize = 1e-3, maxIter = 1000,
                  ...
                  asgd.use = FALSE, asgd.start = 1,
                  nIter.1stTune = floor(0.1 * maxIter),
                  nIter.lastTune = maxIter, period.tune = 100,
                  vcrit.tune = c("ewmabv", "av"),
                  ewmabv.beta = NULL, abv_aofv_w = 0.5,
                  nIter.tauNoIncrease = floor(0.6 * maxIter),
                  period.message = 100, verbose = TRUE,
                  recordParams = TRUE, period.record = 20,
...
fpca.sgd <- function(data_generator, obsGrid, inits, meanfun = FALSE, npc=3,
                     tau = NULL, tau.control = list(), 
                     nbatch = 1, stepsize = 1e-3, maxIter = 1000,
                     conv.check = FALSE,
                     nIter.constStepSize = 0, nIter.adam = floor(maxIter),
                     ...
                     asgd.use = FALSE, asgd.start = 1,
                     coord.scaling = FALSE, coord.scaling.start = 1,
                     scales.cap.factor = 5, scales.cap.end = 200,
                     nIter.1stTune = floor(0.1 * maxIter),
                     nIter.lastTune = maxIter, period.tune = 100,
                     vcrit.tune = c("ewmabv", "av"),
                     ewmabv.beta = NULL, abv_aofv_w = 0.5,
  ...
  if (adatau) { tau.history <- tau; tau.selectId <- c()
  } else { tau.history <- NULL; tau.selectId <- NULL }
  ntune <- floor((maxIter-nIter.1stTune) / period.tune) + 1
  if (ntune < 5) {
    ewmabv.beta <- 0.5
  } else {
    ewmabv.beta.candidates <- seq(0.1,0.95,0.05)
    valid.ewmabv.beta.id <- which(ewmabv.beta.candidates^(ntune-1) < 0.5/ntune^1.2)
    if (length(valid.ewmabv.beta.id)==0) {
      idx <- 1
      warning("Improper choice of ewmabv.beta. The tuning period is possibly too large.")
    }
  ...
  vcrit.history <- list(av = array(dim=c(ntau,ntune)),
  ...
  if (nIter.slowerdecay < nIter.constStepSize) nIter.slowerdecay <- nIter.constStepSize
  
  p <- nrow(inits$Theta)
  q <- ncol(inits$Theta)
  while (q < npc) {
    theta_append = runif(p, -1, 1)
    theta_append = manifold.Stiefel.retractSinglePC(theta_append, inits$Theta, G)
  ...
  } else params.history <- NULL
  iter.params <- 1
  
  # TODO: implement online mean estimation
  if (!is.null(theta_mu)) stop("NOT IMPLEMENTED")
  
  # stored stats for adaptive stochastic gradient
  if (all(c("grad_Theta", "grad_eta", "grad_zeta") %in% names(inits))) {
    inits$grad_Theta <- as.matrix(manifold.Stiefel.project(inits$grad_Theta, inits$Theta, G))
  ...
  for (i in 1:maxIter) {
    ...
    if (verbose && (i %% period.message == 0)) message("Iter ", i)
    ...
    for (l in 1:ntau) {
      ...
      if ((!asgd.use) || i < asgd.start) {
        lik <- objfun(Ly, Ltid, asl(Theta,l), lambda[,l], sigma2[l],
                            theta_mu, tau[l], stats = "loss")$lik
        av[l] <- av[l] * (i-1)/i + lik * 1/i
        curr_bv[l] <- curr_bv[l] + lik
      } else {
        lik.avg <- objfun(Ly, Ltid, asl(Theta.avg,l), lambda.avg[,l], sigma2.avg[l],
                          theta_mu, tau[l], stats = "loss")$lik
    ...
    if (i > 1 && i >= nIter.1stTune && (i - nIter.1stTune) %% period.tune == 0) {
      ...
      if (adatau && i <= nIter.lastTune) {
        ...
        tau.control <- setParams.tau(
          tau, tau.control,
          delta.min = taurange0/max(1,ntau-2)/(itune^0.7))$tau.control
        tau <- explore_newtau(tau[lstar], nchild, tau.control$delta,
    ...
    for (l in 1:ntau) {
      ...
      # if (i > nIter.lastTune && l > 1) break
      objective <- objfun(Ly, Ltid, asl(Theta,l), lambda[,l], sigma2[l],
                          theta_mu, tau[l], stats = "grad")
      grad_Theta <- objective$grad_Theta
      grad_Theta <- as.matrix(manifold.Stiefel.project(grad_Theta, asl(Theta,l), G))
      ...
      if (i <= nIter.adam) { # adaptive gradients
        direc <- m[,l] / (sqrt(v[,l]) + 1e-8)
      } else { # switch back to regular sgd
      ...
      if (coord.scaling && i > coord.scaling.start) {
        ...
        } else {lambda.scales <- lambda}
        if (asgd.use && i >= asgd.start) {
          coord.scales <- 1 / c(rep(2*lambda.scales,each=p), lambda.scales, sigma2.avg)
        } else {
          coord.scales <- 1 / c(rep(2*lambda.scales,each=p), lambda.scales, sigma2)
        }
      ...
      if (curr_stepsize < stepsize.min) curr_stepsize <- stepsize.min
      Theta[,,l] <- Theta[,,l] - curr_stepsize * direc[1:(p*q)]
      lambda[,l] <- lambda[,l] * exp(-curr_stepsize * direc[(p*q+1):(p*q+q)])
      sigma2[l] <- sigma2[l] * exp(-curr_stepsize * direc[p*q+q+1])
      
      if (all(curr_stepsize * abs(direc) <
              1e-8 * abs(c(Theta[,,l], log(lambda[,l]), log(sigma2[l])))))
        conv_flag[l] <- TRUE
      
      if (asgd.use && i >= asgd.start) {
        Theta.avg[,,l] <- Theta.avg[,,l] * ((asgd.count-1) / asgd.count) +
          Theta[,,l] / asgd.count
      ...
      m[1:(p*q),l] <- as.matrix(
        manifold.Stiefel.project(matrix(m[1:(p*q),l],p,q), asl(Theta,l), G))
  ...
  out <- list(
    Theta=Theta, lambda=lambda, sigma2=sigma2,
    theta_mu=theta_mu, tau=tau, tau.min=tau.min,
    av=av, ewmabv=ewmabv, 
    tau.select=list(tau.history=tau.history, tau.selectId=tau.selectId, iter.select=iter.select),
    params.history=list(params=params.history, iter.params=iter.params),
    vcrit.history=vcrit.history,
    time.history=time.history, stepsize = stepsize, stepsize.final = stepsize * decay,
    ewmabv.beta = ewmabv.beta, asgd.use = asgd.use
  )
  
  if (asgd.use) {
    out <- append(
      out, list(
        Theta.avg=Theta.avg, lambda.avg=lambda.avg, sigma2.avg=sigma2.avg,
        av.avg=av.avg, ewmabv.avg=ewmabv.avg
      )
    )
  }
...
fpca.rasa <- function(data_generator, obsGrid, inits, meanfun = FALSE,
                     tau = NULL, tau.control = list(), 
                     nbatch = 1, stepsize = 1e-3, maxIter = 1000,
                     conv.check = FALSE,
                     stepsize.decayrate = 0.5, stepsize.min = 0,
                     nIter.slowerdecay = floor(0.5 * maxIter),
                     stepsize.decayrate.slow = 0, asgd.start = 1,
                     beta0 = 0.9, beta1 = 0.9, beta2 = 0.99,
                     nIter.1stTune = floor(0.1 * maxIter),
                     nIter.lastTune = maxIter, period.tune = 100,
                     vcrit.tune = c("ewmabv", "av"),
                     ewmabv.beta = NULL, abv_aofv_w = 0.5,
  ...
  for (i in 1:maxIter) {
    ...
    for (l in 1:ntau) {
      ...
      rasa.l[,l] <- rasa.l[,l]*beta2 + c(diag(grad_Theta %*% t(grad_Theta)))*(1-beta2)/q
      rasa.r[,l] <- rasa.r[,l]*beta2 + c(diag(t(grad_Theta) %*% grad_Theta))*(1-beta2)/p
      m.lamsig[,l] <- m.lamsig[,l] * beta1 + c(grad_eta, grad_zeta) * (1 - beta1)
      v.lamsig[,l] <- v.lamsig[,l] * beta2 + c(grad_eta, grad_zeta)^2 * (1 - beta2)
      direc.Theta <- outer((rasa.l[,l]+1e-8)^(-1/4),(rasa.r[,l]+1e-8)^(-1/4))*matrix(m.Theta[,l],p,q)
      direc.Theta <- as.matrix(manifold.Stiefel.project(direc.Theta, asl(Theta,l), G))
  ...
  out <- list(
    ...
    params.history=list(params=params.history, iter.params=iter.params),
    vcrit.history=vcrit.history,
    time=time, stepsize = stepsize, stepsize.final = stepsize * decay,
...
objfun <- function(Ly, Ltid, Theta, lambda, sigma2, theta_mu = NULL, tau = 1e-3,
  ...
  if (length(tau) == 1 && q > 1) tau <- rep(tau, q)
  ...
  for (i in seq_len(n)) {
    ...
    if (stats == "all" || stats == "loss") {
      ...
      fval <- fval + 1 / n * (1/sigma2 * (sum(Yi^2) - sum(PhiT_Yi * invQ %*% PhiT_Yi)) +
                                (mi-q) * log(sigma2) + sum(log(lambda)) + logdetQ)
    }
    if (stats == "all" || stats == "grad") {
      Phit_Phi <- crossprod(Phi)
      invQ_PhiT_Yi <- invQ %*% PhiT_Yi
      grad_Theta <- grad_Theta + 2 / n * as.matrix(
        crossprod(Bi, Phi) %*% (1/sigma2 * invQ_PhiT_Yi %*% t(invQ_PhiT_Yi) + invQ) - 
          1/sigma2 * crossprod(Bi, Yi) %*% t(invQ_PhiT_Yi)
      )
      grad_eta <- grad_eta + lambda / n * (
        -(1/sigma2 * (PhiT_Yi - Phit_Phi %*% invQ_PhiT_Yi))^2 +
          1/sigma2 * (diag(Phit_Phi) - colSums(Phit_Phi * invQ %*% Phit_Phi))
      )
      grad_zeta <- grad_zeta + 1 / n * (
        -1 / sigma2 * (sum(Yi^2) - 2 * sum(PhiT_Yi * invQ_PhiT_Yi) +
  ...
  if (stats == "all" || stats == "grad") {
    out[["grad_Theta"]] <- grad_Theta + 2 * matrix(rep(tau,each=p),nrow=p) * as.matrix(Omega %*% Theta)
...
setParams.inits <- function(inits, meanfun = FALSE) {
  if (!("Theta" %in% names(inits))) {stop("Lack an initialization for Theta")}
  if (!("lambda" %in% names(inits))) {stop("Lack an initialization for lambda")}
  if (!("sigma2" %in% names(inits))) {stop("Lack an initialization for sigma2")}
  if (meanfun) {
    if (!("theta_mu" %in% names(inits))) {stop("Lack an initialization for theta_mu")}
  } else { inits$theta_mu <- NULL}
  return(inits)
}
...
setParams.tau <- function(tau = NULL, tau.control = list(), delta.min = NULL) {
  if (!is.null(tau)) {
    ...
    if (!"ntau" %in% names(tau.control)) tau.control$ntau <- 9
    if (!"maxtau" %in% names(tau.control)) tau.control$maxtau <- 1e2
    if (!"mintau" %in% names(tau.control)) tau.control$mintau <- 1e-6
    tau <- with(tau.control, exp(seq(log(mintau),log(maxtau),length.out=ntau)))
  }
  if (!"adatau" %in% names(tau.control)) tau.control$adatau <- TRUE
  if (tau.control$adatau) {
    if (!"nselect" %in% names(tau.control)) tau.control$nselect <- 3
    if (tau.control$nselect > tau.control$ntau) tau.control$nselect <- tau.control$ntau
    if (tau.control$nselect > tau.control$ntau / 2)
      warning("nselect > ntau/2 is not effective to explore the parameter space.")
    tau.control$delta <- mean(diff(sort(log(tau)))) / 3
    if (is.null(delta.min)) {delta.min <- 0.3
    } else {delta.min <- max(delta.min, 0.3)}
...
explore_newtau <- function(tau, nchild, delta, noincrease = FALSE) {
  ...
  for (l in seq_along(tau)) {
    if (noincrease) {
      newtau <- c(newtau, tau[l] * exp(-seq(0,2*delta,length.out=nchild[l])))
    } else
    newtau <- c(newtau, tau[l] * exp(-seq(-delta,delta,length.out=nchild[l])))
  }
  return(newtau)
}


plot.tau_path <- function(tau.history, tau.selectId,
                          col.selected="royalblue3", col.discarded="gray50",
                          lty.selected=1, lty.discarded=2,
                          lwd.selected=1.5, lwd.discarded=1) {
  ...
  plot(x=NULL, xlim=c(0, d), ylim=c(lylim, uylim),
       xlab=expression(Number~of~Data~Blocks), ylab=expression(log[10](tau)))
  for (i in d:1) {
    taui <- tau.history[,i+1]
    for (j in 1:length(taui)) {
      if (i < d && (!j %in% tau.selectId[,i+1])) {
        curr_col <- col.discarded; curr_lty <- lty.discarded; curr_lwd <- lwd.discarded
      } else {
        curr_col <- col.selected; curr_lty <- lty.selected; curr_lwd <- lwd.selected}
      lines(x = c(i-1, i), type = "b", col=curr_col, lty=curr_lty, lwd = curr_lwd,
  ...
  points(x=rep(0,length(tau1.selected)), y=log10(tau1.selected),
         col=col.selected, lty=lty.selected, lwd=lwd.selected)
  points(x=rep(0,length(tau1.discarded)), y=log10(tau1.discarded),
         col=col.discarded, lty=lty.discarded, lwd=lwd.discarded)
}


plot.tau_path2 <- function(
  tau.history, tau.selectId, final.id = 1, nbatch_per_block = 1,
  col.selected="royalblue3", col.discarded="gray60",
  lty.selected=1, lty.discarded=2,
  lwd.selected=1.5, lwd.discarded=1,
  plot.title = NULL) {
  d <- ncol(tau.history) - 1
  ...
  uylim <- maxlogtau + 0.05 * (maxlogtau - minlogtau)
  lylim <- minlogtau - 0.05 * (maxlogtau - minlogtau)
  plot(x=NULL, xlim=c(0, d * nbatch_per_block), ylim=c(lylim, uylim),
     xlab=expression(Iteration~k), ylab=expression(log[10](tau[k])),
    main = plot.title)
  # draw all paths in grey
  for (i in d:1) {
    taui <- tau.history[,i+1]
    for (j in 1:length(taui)) {
      lines(x = c(i-1, i) * nbatch_per_block, type = "l", col=col.discarded,
            lty=lty.discarded, lwd = lwd.discarded,
            y = c(log10(tau.history[tau.selectId[j,i],i]), log10(taui[j])))
    }
    points(rep(i, w) * nbatch_per_block, log10(taui), pch=21,
           col=col.discarded, lwd=lwd.discarded, bg="white")
  }
  points(rep(0, w) * nbatch_per_block, log10(tau.history[,1]), pch=21,
         col=col.discarded, lwd=lwd.discarded, bg="white")
  # draw the selected path in blue
  tau_path <- extract_tau_path(tau.history, tau.selectId, final.id)
  for (i in d:1) {
    lines(x = c(i-1, i) * nbatch_per_block, type = "l", col=col.selected,
          lty=lty.selected, lwd = lwd.selected,
          y = log10(c(tau_path$tau_path[i], tau_path$tau_path[i+1])))
    points(i * nbatch_per_block, log10(tau_path$tau_path[i+1]), pch=21,
           col=col.selected, lwd=lwd.selected, bg="white")
  }
  points(0 * nbatch_per_block, log10(tau_path$tau_path[1]), pch=21,
         col=col.selected, lwd=lwd.selected, bg="white")
}
