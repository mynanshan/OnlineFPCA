...
# - Fits functional principal components for sparse/irregular observations
#   using an alternating update between component loadings (`Theta`) and
#   per-subject scores (`alphamat`). Supports tuning `tau` via GCV or an
#   optional validation set, and records iteration histories if requested.
...
fpca.reg <- function(
  ...
  # 1) for each FPC (k = 1..q) and each `tau` candidate, iterate updates of
  #    thetak (loading) and alphak (scores) until convergence;
  # 2) compute GCV (or validation) scores to select best `tau` per component;
  # 3) store selected Theta and compute final alphamat.

  tau_list <- sort(tau)
  ntau <- length(tau_list)

  N <- length(Ly)
  # Optionally reserve a validation set (fraction `n_valid_ratio`) for tuning
  ...
  if (!is.matrix(Theta) && is.vector(Theta)) {
    Theta <- matrix(Theta, ncol = 1)
  }

  p <- nrow(Theta)
  q <- ncol(Theta)
  # Ensure we have at least `npc` initial components; append random
  # Stiefel-retracted directions as needed (keeps orthogonality w.r.t. G)
  while (q < npc) {
  ...
  if (record_iterations) {
    objvals <- array(NA, dim = c(q, nrecord, ntau))
  }
  if (record_iterations) {
    Theta_history <- array(NA, dim = c(p, q, nrecord, ntau))
  }
  ...
  theta_gcv_factor2 <- 1 -
    ...
    ) # less N, higher mi, higher difficulty for theta
  ...
  # Main iterations over principal components -----------------------------
  # For each component k: iterate over tau candidates and update thetak using
  # alternating closed-form updates for alphak (scores) and thetak (loadings).

  for (k in 1:q) {
    if (verbose) {
      message("Update FPC ", k)
    }
    ...
    for (itau in seq_along(tau_list)) {
      ...
      for (ii in 1:maxIter) {
        ...
        # Compute FPC scores `alphak` for current thetak using a ridge-like
        # closed-form update (see `fpca.reg.getAlphak`).
        ...
        # Accumulate normal-equation components for solving thetak update:
        # M approximates sum(alpha^2 B_i^T B_i / m_i) and u approximates
        # sum(alpha B_i^T z_i / m_i). These form a linear system (M + pen)
        # * thetak = u.
        ...
        # update fpc[k]
        thetak <- solve(
          as.matrix(M + tau * mean(alphak^2 * Lmi^(-1 / (1 + nu))) / N * Omega),
          u
        )

        # Re-orthonormalize `thetak` against previously computed components
        # (weighted by Gram matrix `G`), then scale to unit G-norm.
        if (k > 1) {
          inprod_phik_phi <- as.matrix(crossprod(
            Theta[, 1:(k - 1), drop = F],
            G %*% thetak
          ))
        ...
        if (record_iterations && (ii %% record_period == 0)) {
          ...
          objvals[k, ir, itau] <- mapply(
            function(Zi, alpha_ik, phi_ik) {
        ...
        # check convergence
        if (
          max(abs(thetak - thetak_old)) / max(abs(thetak_old)) < 1e-4 ||
      ...
      if (verbose && ii == maxIter) {
        message("FPC ", k, " has not converged")
      }
      conv_flag[k, itau] <- !(ii == maxIter) * 1
      nIter[k, itau] <- ii

      # Compute GCV-related statistics used to tune `tau`:
      # - wsse: weighted SSE across subjects
      wsse <- mapply(
        function(Zi, alpha_ik, phi_ik) {
    ...
    # Finalize the FPC scores for component k. By default use the closed-form
    # `fpca.reg.getAlphak` update with the selected `tau`; if `refine_alpha` is
    # TRUE, fit a ridge regression per subject to refine scores.
    if (!refine_alpha) {
      ...
      alphamat[, k] <- mapply(
        function(Zi, phi_ik) {
          fastmatrix::ridge(
            y ~ x - 1,
            data = data.frame(y = Lz[[i]], x = Lphik[[i]])
          )$coefficients
        },
        Lz,
        Lphik
  ...
  # Return a list containing estimated components (`Theta`), per-subject
  # scores (`alphamat`), selected taus and diagnostics/statistics useful for
  # tuning and assessing fit.
  return(list(
    Theta = Theta,
    theta_mu = theta_mu,
    alphamat = alphamat,
    ...
    gcv_scores_alpha = gcv_scores_alpha,
    gcv_scores_theta = gcv_scores_theta,
    conv_flag = conv_flag,
    theta_gcv_factor = theta_gcv_factor,
    test_scores = if (use_validation_set) {
      test_scores
    } else {
      NULL
    },
