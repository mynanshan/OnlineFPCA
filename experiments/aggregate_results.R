...
renewIds <- function(dat, startId=1, Id="iRep") {
  uniqueIds <- sort(unique(dat[[Id]]))
  newIds <- startId:(startId+length(uniqueIds)-1)
  return(list(newIdvec = newIds[match(dat[[Id]], uniqueIds)],
              newIds = newIds))
}

read_experiments <- function(filenames, Id="iRep") {
  dat <- read.csv(filenames[1])
  obj <- renewIds(dat, 1, Id)
  dat[[Id]] <- obj$newIdvec
  maxId <- max(obj$newIds)
  if (length(filenames) > 1) {
    for (i in 2:length(filenames)) {
      newdat <- read.csv(filenames[i])
      obj <- renewIds(newdat, maxId+1, Id)
      newdat[[Id]] <- obj$newIdvec
      maxId <- max(obj$newIds)
...
sig <- "0p5"
filenameprefix <- c(
  paste0("fpca1d_alpha",a,"_noise",sig,"_"),
  paste0("soap1d_alpha",a,"_noise",sig,"_")
)
