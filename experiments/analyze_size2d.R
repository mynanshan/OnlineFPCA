...
recode_method <- function(Method) {
  dplyr::case_when(
    Method == "OnlineFPCA-sgd" ~ "OnlineFPCA-RSGD",
    Method == "OnlineFPCA-adagrad" ~ "OnlineFPCA-RAdagrad",
    Method == "Batch-SOAP" ~ "SOAP",
    grepl("^Batch-mOpCov-N", Method) ~ "mOpCov",
    TRUE ~ Method
  )
}
...
sumres <- res |>
  ...
  mutate(SumTime = round(SumTime, 1)) |>
  rename(
...
ggsave(
  file.path(dirpath, "size2d_rmse.pdf"),
  p,
  width = 8.4,
  height = 4.2,
  dpi = 300,
