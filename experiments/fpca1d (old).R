  ...
  for (noise_sd in noiseList) {
    ...
    filename <- file.path(
      ...
      paste0(
        exprmt,
        "_alpha",
        stringr::str_replace(alpha, "\\.", "p"),
        "_noise",
        stringr::str_replace(noise_sd, "\\.", "p"),
        "_",
        stringr::str_remove_all(
          as.character(round(as.numeric(Sys.time()))),
          "[ \\-:]"
        ),
        ".csv"
      )
    ...
    for (ir in 1:nRep) {
          ...
          for (q in npcList) {
            ...
            for (nBatch in nBatchList) {
              for (stepsize in stepsizeList) {
                ...
                PhiEstEval <- match_fpc(
                  PhiEstEval,
                  PhiTrueEval[, 1:q, drop = F]
                )
                ...
                tau_path_id_extend <- c(
                  rep(tau_path_id[1], nRoundNoTune),
                  tau_path_id
                )
      ...
      PhiEstAll.ll <- sapply(
        1:Kmax,
        \(k) matrix(fit.ll$vecs[1:(qmax * EV2), k], ncol = qmax),
        simplify = "array"
      )
      PhiEst.ll <- PhiEstAll.ll[,, Kmax]
      lambdaEst.ll <- fit.ll$vals[1:qmax, Kmax]
      PhiEstFunc.ll <- smooth_basis(
        eval_gam_vec,
        PhiEst.ll,
        basis,
        lambda = 1e-10
      ...
      PhiBatchEval <- flip_direc(
        t(fitBatch$eigenfunctions),
        PhiTrueEval[, 1:qmax]
      )
      ThetaBatch <- smooth_basis(
        evalGrid,
