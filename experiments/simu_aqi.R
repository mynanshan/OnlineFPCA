...
CovRes <- mOpCov(
  location=Tinit, x=Yinit, subject=subject_ids,
...
for (optMethod in c("SGD", "Adam")) {
  ...
  fit <- fpca.sgd(
    ...
    tau = NULL, tau.control = list(
      ntau=nParams, nselect=2, maxtau=1e-1, mintau=1e-8), 
...
fitBatch <- fpca.reg(
  dat$Ly, dat$Ltid, inits = inits,
  meanfun = FALSE, npc=npc, maxIter = 100, nu=0.5,
