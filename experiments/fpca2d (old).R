...
for (alpha in alphaList) {
  for (noise_sd in noiseList) {
    ...
    for (ir in 1:nRep) {
      ...
      for (nBatch in nBatchList) {
        for (stepsize in stepsizeList) {
          ...
          fit <- fpca.sgd(
            ...
            meanfun = FALSE,
            tau = NULL, tau.control = list(ntau=nParams, nselect=2, maxtau=1e-3, mintau=1e-8), 
      ...
      fitBatch <- fpca.reg(
        dat$Ly, dat$Ltid, inits = list(
          Theta = ThetaInit, lambda = lambdaInit, sigma2 = sigma2Init),
        meanfun = FALSE, npc=3, maxIter = 100, nu=0.5,
        verbose = FALSE, record_iterations = TRUE,
        tau=10^seq(-9, -4, 1), use_validation_set = FALSE,
        refine_alpha = FALSE)
