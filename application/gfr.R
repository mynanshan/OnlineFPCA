...
dat$Ly <- unname(apply(as.matrix(dat[,3:9]), 1, \(x) (x-gfr_mean)[!is.na(x)], simplify = FALSE))
dat$Ltid <- unname(apply(as.matrix(dat[,3:9]), 1, \(x) which(!is.na(x)), simplify = FALSE))
dat$Lmi <- sapply(dat$Ltid, length)
# rm(dat)

barplot(table(dat$Lmi) / length(dat$Lmi) * 100,
  xlab="Number of Records", ylab="Percent of Recipients (%)")
mean(dat$Lmi == 7)
mean(dat$Lmi == 6)
mean(dat$Lmi < 6)
...
matplot(PhiInitEval, type="l", lty = 3, xlab="t", ylab=expression(phi[k](t)),
        main="Initial Estimates")
sigma2Init <- ppcaObj$mle.sigma2

## Online FPCA ==============

message("Start Adam FPCA")

fit <- fpca.sgd(
  fdata_generator, tgrid, inits = list(
    Theta = ThetaInit, lambda = lambdaInit, sigma2 = sigma2Init),
  meanfun = FALSE,
  tau = NULL, tau.control = list(ntau=nParams, nselect=1, maxtau=1e-1, mintau=1e-4),
...
fit.ll <- fpca.lpoly.online(
  ...
  L2 = nParams, G = 0.5, R = 1, Mcl = 1, C = 1.5,
  verbose = TRUE, period = 1)

# check eigenfunctions
PhiEstAll.ll <- sapply(1:Kmax, \(k) matrix(fit.ll$vecs[1:(q*EV2),k],ncol=q), simplify = "array")
...
ThetaAll.avg[,attributes(PhiAvgEval)$flipped,] <-
  -ThetaAll.avg[,attributes(PhiAvgEval)$flipped,]
PhiAvgAll <- array(apply(ThetaAll.avg, 3, \(X) eval_fd(evalGrid, FuncData(X, basis))),
...
save(PhiAvgEval, PhiEst.ll, PhiAvgAll, PhiEstAll.ll, lambda.avg, lambdaEst.ll,
     sgd_time, loclin_time, tau.select, file = "application/result_gfr.Rdata")
