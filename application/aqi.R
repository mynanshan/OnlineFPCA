...
fit <- fpca.sgd(
  ...
  tau = NULL, tau.control = list(
    ntau=nParams, nselect=2, maxtau=10^taumax, mintau=10^(taumax-nParams+1)),
  nbatch = nBatch, maxIter = nPass*nIter1pass,
  stepsize = stepsize, stepsize.decayrate = 0.6, stepsize.min = 1e-3,
  ...
  period.tune = nBlockIter, period.record = nBlockIter, verbose = TRUE)
...
fitBatch <- fpca.reg(
  fdata_generator(N, 0)$Ly, fdata_generator(N, 0)$Ltid, inits = list(
    Theta = ThetaInit, lambda = lambdaInit, sigma2 = sigma2Init),
  meanfun = FALSE, npc=6, maxIter = 300, nu=0.5,
  verbose = TRUE, record_iterations = TRUE,
  tau=10^seq(-6, -6, 1), use_validation_set = FALSE,
  refine_alpha = FALSE)
batch_end <- Sys.time()
