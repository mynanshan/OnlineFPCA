...
all_lims <- sapply(1:q, \(k) {
  range(c(
    PhiAvgAll[-(1:4), k, -(1:check_points[1])],
    PhiEstAll.ll[-(1:4), k, -(1:check_points[1])],
    resCI[-(1:4), k, , -(1:check_points[1])]
  ) |> na.omit())
})
...
add_ci <- TRUE
layout(
  mat = matrix(c(1:npanel, rep(npanel + 1, q)),
    byrow = TRUE,
...
for (idx in seq_along(check_points)) {
  for (k in 1:q) {
    ...
    if (idx == 1) {
      mtext(
        paste0("FPC ", k, ", ", round(fve[k] * 100, 1), "%"),
        side = 3, line = 1
      )
    }
    if (idx == 3 && k == 2) {
...
legend("bottom",
  legend = c("OnlineFPCA-RSGD", "OnlineCov"), inset = c(0, 0),
