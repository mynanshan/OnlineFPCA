...
usa <- giscoR::gisco_get_countries(country="USA") |>
  sf::st_crop(c(xmin = lonrange[1], xmax = lonrange[2],
...
ggplot() +
  geom_sf(data=us_states) +
  geom_point(aes(x=Longitude, y=Latitude, color=Nrecord,
                 size=Nrecord, group=Nrecord), stroke=1,
...
gridwidth <- 2
rawgrid <- expand.grid(
  list(Latitude=seq(latrange[1]+gridwidth/2,latrange[2]-gridwidth/2,gridwidth),
...
rawgrid[["InUS"]] <- sapply(1:nrow(rawgrid), \(i) {
  ll <- c(rawgrid$Longitude[i], rawgrid$Latitude[i]) |>
    st_point() |> st_sfc(crs=espg) |>
    st_intersects(us_states) |>
    unlist() |> length()
  return(ll > 0) })
latlongrid <- rawgrid |> filter(InUS) |> dplyr::select(Latitude, Longitude)
...
ggplot() +
  geom_sf(data=us_states) +
  geom_point(aes(x=Longitude.Binned, y=Latitude.Binned, color=Nrecord,
...
x <- dat |> filter(Latitude.Binned==33, Longitude.Binned==-87) |>
  pull(AQI) |> log1p() 
acf(diff(x))
...
acfcheck <- dat |> 
  ...
  group_by(Latitude.Binned, Longitude.Binned) |>
  summarise(acf1.max = max(abs(acf(logAQI,plot=F)$acf[-1])),
...
date_remove <- dat |> group_by(Date.Local) |> summarise(Nobs=n()) |>
  filter(Nobs==1) |> pull(Date.Local)
    ...
    dat <- dat |>
      ...
      left_join(siteinfo |>
                  dplyr::select(Latitude, Longitude,
                                Latitude.Binned, Longitude.Binned),
                by = c("Latitude", "Longitude")) |>
      dplyr::select(Latitude, Longitude, Latitude.Binned, Longitude.Binned, Date.Local, AQI) |>
      mutate(Date.Local = ymd(Date.Local)) |>
      arrange(Date.Local)
    return(dat)
  }) )
...
plots = lapply(
  selected_dates, \(d) {
    ggplot() +
      geom_sf(data=us_states) +
      geom_point(aes(x=Longitude, y=Latitude), stroke=1,
...
ggsave(filename = "text-onlineFPCA/figures/api-sample.pdf", fig, width = 8, height=3,
       device = "pdf")
...
Lmi <- dat |> group_by(Date.Local) |>
  summarise(Nobs=n()) |> pull(Nobs)
